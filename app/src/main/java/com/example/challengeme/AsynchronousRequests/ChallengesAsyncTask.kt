...
import com.example.challengeme.Interfaces.PostChallenges
...
class ChallengesAsyncTask(val id: Int) : AsyncTask<String, Void, ArrayList<UserChallenge>?>() {
    ...
    override fun doInBackground(vararg p0: String?): ArrayList<UserChallenge>? {
        ...
        //val postChallenges: PostChallenges = retrofit.create(PostChallenges::class.java)
        try {
            //val challengeObj: Call<ArrayList<UserChallenge>?>? = getChallenges.getChallenges()
            /*val challengeObj = postChallenges.postChallenges(mapOf("id" to id))
